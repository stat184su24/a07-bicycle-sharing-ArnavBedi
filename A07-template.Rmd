...
```{r}
# Converting sdate and edate to POSIXct format if not already done
Trips$sdate <- as.POSIXct(Trips$sdate, format="%Y-%m-%d %H:%M:%S")
Trips$edate <- as.POSIXct(Trips$edate, format="%Y-%m-%d %H:%M:%S")

# Calculating the duration in minutes
Trips <- Trips %>%
  mutate(duration = as.numeric(difftime(edate, sdate, units = "mins")))

# Calculating the 95th percentile to filter out long duration outliers
percentile_95 <- quantile(Trips$duration, 0.95)

# Filtering out trips longer than the 95th percentile
Trips_filtered <- Trips %>%
  filter(duration <= percentile_95)

# Creating the box-and-whisker plot
ggplot(Trips_filtered, aes(x = client, y = duration, fill = client)) +
  geom_boxplot() +
  labs(title = "Distribution of Rental Durations by Client Type",
       x = "Client Type",
       y = "Duration (minutes)") +
  theme_minimal()
...
### Day of Year
```{r}
# Histogram for Day of the Year
ggplot(Trips, aes(x = day_of_year)) +
  geom_histogram(binwidth = 1, fill = "pink", color = "black") +
  labs(title = "Bike Check-outs by Day of the Year",
       x = "Day of the Year",
       y = "Count") +
  theme_minimal()
...
My observations here are that the graph shows that the maximum amount of bike checkouts happen around the 275th day of the year and gradually decreases 
...
ggplot(Trips, aes(x = day_of_week)) +
  geom_histogram(stat = "count", fill = "purple", color = "black") +
  labs(title = "Bike Check-outs by Day of the Week",
       x = "Day of the Week",
       y = "Count") +
  theme_minimal()
...
My observations here are that there is not much difference between the amount of checkouts on the days of the week with the maximum being on Thursdays and Fridays.  
...
```{r}
ggplot(Trips, aes(x = hour_of_day)) +
  geom_histogram(binwidth = 1, fill = "red", color = "black") +
  labs(title = "Bike Check-outs by Hour of the Day",
       x = "Hour of the Day",
       y = "Count") +
  theme_minimal()
...
My observations here that there is a pretty significant difference in the checkouts by hour of the day where the highest being around 8am followed by 5pm, and the lowest being between 2am to 5am. 
...
```{r}
ggplot(Trips, aes(x = minute_in_hour)) +
  geom_histogram(binwidth = 1, fill = "orange", color = "black") +
  labs(title = "Bike Check-outs by Minute in the Hour",
...
My observations here are that there is not much difference in which minute of the hour the checkouts happen with the average being around 150 to 200 bikes per minute in an hour 
...
# Ploting the data
ggplot(summary_data, aes(x = hour_of_day, y = count, color = client, group = client)) +
  geom_line() +
...
By hour of the day : There are clear peaks in bike rentals during the morning (around 8-9 AM) and evening (around 5-6 PM) hours, which align with typical commuting times.

By day of the week : Weekday patterns (Monday to Friday) show strong peaks during commute hours and 
Weekend patterns (Saturday and Sunday) are more evenly distributed throughout the day, with fewer sharp peaks, indicating more casual use.

By client type comparison : Regular users/members show more pronounced peaks during commuting hours, indicating a higher usage for daily commutes whereas casual users have a more even distribution, with slightly higher usage during weekends and off-peak hours, suggesting more recreational/casual use.
...
```{r}
# Joining with Trips to calculate start-to-end distance of each trip
TripsDistances <- Trips %>%
  inner_join(PairDistances, by = c("sstation" = "sstation", "estation" = "estation"))

# Ploting distribution of ride distances
ggplot(TripsDistances, aes(x = distance)) +
...
# Ploting distribution of distances between station pairs
ggplot(PairDistances, aes(x = distance)) +
  geom_density(fill = "red", alpha = 0.5) +
  labs(title = "Distribution of Distances Between Station Pairs",
       x = "Distance (km)",
       y = "Density") +
  theme_minimal()

# Overlaying the two density plots
ggplot() +
  geom_density(data = TripsDistances, aes(x = distance), fill = "blue", alpha = 0.5) +
  geom_density(data = PairDistances, aes(x = distance), fill = "red", alpha = 0.5) +
  labs(title = "Comparison of Ride Distances and Station Pair Distances",
       x = "Distance (km)",
       y = "Density") +
  theme_minimal()
